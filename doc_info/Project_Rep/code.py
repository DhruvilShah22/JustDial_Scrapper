...
requirements = ['Name', 'Specialties', 'Location', 'Overall Rating', "About", "Office location", 'Practice names',
                'Hospital affiliations', 'Board certifications', 'Education and training', 'Professional memberships',
                'Languages spoken', "Provider's gender", 'NPI number']
df = pd.DataFrame(columns=requirements)
...
def chrome(url):
    ...
    try:
        try:
            driver.find_element_by_xpath(
                "/html/body/div/div/main/div[1]/div[3]/div[1]/section/section[1]/span/div/button").click()
            info = driver.find_element_by_xpath(
                ".//html/body/div/div/main/div[1]/div[3]/div[1]/section/section[1]/span/div/span").text
        except:
            info = driver.find_element_by_xpath(
                "./html/body/div/div/main/div[1]/div[3]/div[1]/section/section[1]/span/div/span").text
    except:
    ...
    i = [-1] * 14
    for ind, x in zip(range(0, 14), requirements):
        if x in req:
            i[ind] = req.index(x)
